...
public class ActiveLabel: UILabel {
    ...
    private func parseTextAndExtractActiveElements(attrString: NSAttributedString) {
        ...
        textString.componentsSeparatedByCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet()).forEach { word in
            var element = activeElement(word)
            if let delegate = delegate where delegate.label(self, shouldIgnoreElement: element) {
                element = .None
            }

            if case .None = element { return }
